...
const listkey = ["zeeoneofc", "ahmadamin"];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------BastBotz'+'BastBotz';
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'ahmadamin') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'ahmadamin')  return res.json(loghandler.invalidKey)
